...
public class Game {
    ...
    public Game(int numOfTurn, int initialCash) {
        setNumOfTurn(numOfTurn);
        setNumOfPlayer(NUM_OF_PLAYERS);
        setNumOfAi(NUM_OF_PLAYERS - 1);
        setNumOfSurvival(NUM_OF_PLAYERS);
    ...
    /** New **/
    public ArrayList<String> setUpGame() {
        ...
        for(Player player: this.players){
            Card pocketCards[] = player.getPocket();
            for(Card card: pocketCards){
                cards.add(card.getCardStr());
            }
        }
    ...
    public void simulateOneTurn(){
        if (!isWinnerFind) {
            action(currentTurnCount);
        }
        if (currentTurnCount == 0) {  // flip three cards after turn 0
            pool.flipThreeCards();
        }
    ...
    public boolean isPlayerHasEnoughMoney() {
        ...
        if (!players[0].isSurival()) {
            System.out.println("User do not have enough money to play");
    ...
    public void action(int turn) {
        ...
        if (turn == 0) {
            ...
            highestBet = players[counter].getCurrentBet();
        ...
        while(!(isOneRound && checkEqual())) {
            ...
            if (getHighestBet() == players[counter].getCurrentBet()) {
                canCheck = true;
            }
            else {
                canCheck = false;
...
/**
            switch (input) {
                case 1:
                    ...
                    players[counter].raise(highestBet,input);
                ...
                case 3:
                    ...
                    gap = getHighestBet() - players[counter].getCurrentBet();
                    players[counter].call(getHighestBet());
...
**/
    ...
    public boolean checkEqual() {
        ...
        for (i = 0; i < numOfPlayer; i++) {
            if (players[i].isSurival()) {
                if (players[i].getCurrentBet() != getHighestBet()) {
                    return false;
                }
            }
        }
    ...
    /**
     * @param highestBet the currentCash to set
     */
    ...
    /**
     * @param start idx the endIndex to set
     */
